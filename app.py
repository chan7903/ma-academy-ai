                ...
                if st.button("✨ AI 분석 시작", type="primary"):
                    with st.spinner("AI 선생님이 문제를 분석 중입니다..."):
                        try:
                            ...
                            prompt = f"""
                            당신은 대한민국 수학 교육과정(2015/2022 개정)에 정통한 대치동 1타 강사입니다. 
                            과목:{selected_subject}, 말투:{tone}
                            이미지를 분석하여 정보를 JSON으로 주세요.

                            **[필수 준수 사항: 교육과정 및 용어]**
                            1. **대학 과정 용어 절대 금지:** 'mod', '합동식', '편미분', '로피탈', '외적' 등 교육과정 외 용어 사용 시 감점.
                            2. **고교 과정 정석 용어 사용:** - 'mod' 대신 **'나머지정리'** 또는 **'검산식(A = BQ + R)'**을 사용하세요.
                               - 풀이는 해당 학년({selected_subject}) 수준에 맞춰야 합니다.
                            3. **TMI 금지:** 인사말, 서론 생략. 논리 흐름(→, ∴) 위주로 불친절하고 간결하게 작성.

                            **[수식 및 JSON 문법 가이드]**
                            1. **LaTeX 수식의 역슬래시(\)는 반드시 두 번(\\) 써야 합니다.** (예: \\\\frac)
                            2. 모든 텍스트 내의 줄바꿈은 `\\n`을 사용하세요.
                            3. 응답은 오직 순수 JSON 문자열만 반환하세요.

                            **[출력 데이터 구조]**
                            {{
                                "formula": "이미지 속 수식 (LaTeX는 $...$로 감싸고 역슬래시는 \\\\ 두번)",
                                "concept": "핵심 개념 (단원명, 예: 나머지정리, 인수분해)",
                                "solution": "풀이 과정 (교육과정 준수, 단계별 \\n 구분, 수식은 $LaTeX$)",
                                "shortcut": "숏컷 공식 (수식은 $LaTeX$)",
                                "correction": "틀린 이유 (간결하게)",
                                "hint_for_image": "오답노트 이미지용 텍스트 (한글 위주 3줄)",
                                "twin_problem": "쌍둥이 문제 (수식 포함)",
                                "twin_answer": "정답 및 해설"
                            }}
                            """
