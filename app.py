...
# 2. JSON 출력을 위한 프롬프트 (수식 렌더링 강화 버전)
                            ...
                            **[수식 및 출력 가이드라인 - 엄수]**
                            1. **모든 수식, 기호, 변수(x, y 등), 숫자는 반드시 달러 기호($)로 감싸야 합니다.** - 나쁜 예: f(x) = x^2 + 2x
                               - 좋은 예: $f(x) = x^2 + 2x$
                            2. 텍스트와 수식 사이는 한 칸 띄우세요.
                            3. 줄바꿈이 필요한 곳에는 반드시 `\\n`을 넣으세요.
                            4. TMI 금지, 인사말 생략. 논리 흐름(→, ∴) 위주로 작성.

                            **[출력 포맷 (JSON)]**
                            {{
                                "formula": "이미지 속 수식 (전체 수식을 $...$로 감싸서)",
                                "concept": "핵심 개념 (단원명)",
                                "solution": "풀이 과정 (각 단계마다 줄바꿈 \\n 필수. 모든 수식은 $LaTeX$ 형식)",
                                "shortcut": "숏컷 공식 (공식 전체를 $...$로 감싸서)",
                                "correction": "틀린 이유 (간결하게)",
                                "hint_for_image": "오답노트 이미지용 텍스트 (수식 제외하고 한글 위주 3줄)",
                                "twin_problem": "쌍둥이 문제 (문제 텍스트와 $수식$ 혼용)",
                                "twin_answer": "정답 및 해설 (정답: $...$ 형식)"
                            }}
                            
                            응답은 오직 JSON 데이터만 포함하세요.
                            """
